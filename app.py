...
ADMIN_HTML_PARTS.append(
    ...
    <script>
        ...
        function formatPhoneValue(value){const normalized=normalizePhoneDigits(value);if(normalized)return `+92 ${normalized}`;const digits=extractPhoneDigits(value).slice(0,10);return digits?`+92 ${digits}`:''}
        function formatEnrollmentPhoneDraftValue(value){return extractPhoneDigits(value).slice(0,10)}
        function formatEnrollmentPhoneInput(input,{finalize=false}={}){if(!input)return;input.value=finalize?formatPhoneValue(input.value):formatEnrollmentPhoneDraftValue(input.value)}
